...
=== "`setfacl`"
    ...
    **`<parameter>`参数说明**:
    ...
    ```
    setfacl [options] <parameter> <filename>
    ```

    |0|1|2|
    |-|-|-|
    |`u`/`g`/`o`/`m`|`<user_name>`/`<group_name>`|`<permission>`|
    ...
    1. 掩码(mask)在ACL中是一个重要的概念, 用于设置文件或者目录的最大权限. 如设置`setfacl -m m::r filename`之后, 该文件的用户, 组和其他人拥有的最大权限都是读, 如果这个时候再设置`setfacl -m u:wenzexu:rw filename`, 那么该用户虽然设置了`rw`但是由于掩码, 无法写只能读. 掩码的影响范围: 
        <ol>
            <li>所有非文件所有者的用户条目</li>
            <li>所有组条目和默认条目</li>
        </ol>
    ...
    **`[options]`参数说明**: 
    ...
    - `-d`: 设置默认ACL条目, 影响将来新创建的文件和子目录, 不会改变现有文件和子目录的权限(仅用于目录)

    ??? example "例子"
    
        ```
        setfacl -m u:john:rw file.txt
        setfacl -m -d g:staff:r example_dir # getfacl example_dir的结果中含有default:group:staff:r--
        setfacl -m m::r filename # 设置文件掩码, 即最大权限为读
        ```
